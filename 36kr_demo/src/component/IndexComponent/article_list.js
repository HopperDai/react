...
class ArticleListItem extends Component {
    ...
    render() {
        let data = this.props.article_data;
        return (
        <li>
...
class ArticleList extends Component {
    ...
    async getData(curPage=0) {
        let cur = await (await fetch(`http://localhost:8090/list?page=${curPage}`)).json();
        let articles = this.state.articles.concat(cur);
        this.setState({
            articles,
            page: curPage
        });
